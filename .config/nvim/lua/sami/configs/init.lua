return {
    require('sami.configs.telescope'),
    require('sami.configs.treesitter'),
    require('sami.configs.lsp'),
    require('sami.configs.nvim-cmp'),
    require('sami.configs.mason'),
    require('sami.configs.statusline'),
    require('sami.configs.noice'),
    require('sami.configs.oil'),
    -- require('sami.configs.nx')
    -- require('sami.configs.bufferline'),
    require('sami.configs.colorscheme'),
    require('sami.configs.indent'),
    -- require('sami.configs.spectre'),
    -- require('sami.configs.toggleterm'),
    require('sami.configs.comment'),
    require('sami.configs.nvim-lint')
}
